...
def to_nepali_date(value, format_str="%Y-%m-%d"):
    ...
    try:
        ...
        result = result.replace(
            "%b", get_nepali_month_name(nepali_date.month, "en")[:3]
        )
...
def nepali_date_picker(
    ...
    **kwargs,
    ...
    Supports all dynamic options available in NepaliDatePickerWidget.

    Example::

        {% nepali_date_picker "my_date" disable_past_dates=True admin_theme=True %}
        {% nepali_date_picker "my_date" disable_weekends=True disable_holidays=True %}
    """
    options = {
        "mode": mode,
        "language": language,
        ...
        "disableHolidays": disable_holidays,
    }
    ...
    if disabled_dates:
        options["disabledDates"] = [
            d.strip() for d in disabled_dates.split(",") if d.strip()
        ]
