...
impl LeapMonthDetector {
    ...
    pub fn find_adhika_masa(bs_year: i32) -> Result<Vec<AdhikaMasa>, String> {
        ...
        let next_mesh = SankrantiFinder::find_sankranti(0, 
            sankrantis[11].julian_day.add_days(25.0))?;
        if let Some(adhika) = Self::check_interval(12, sankrantis[11].julian_day, next_mesh.julian_day)? {
            results.push(adhika);
        }
    ...
    fn check_interval(month_idx: u8, start_s: JulianDay, end_s: JulianDay) -> Result<Option<AdhikaMasa>, String> {
